...
# def sacar_unaSinopsis(film, header): 
#     df_movies = pd.read_csv('movies.csv') 
#     df_links = pd.read_csv('links.csv')
#     # df_movieLinks= pd.merge(df_movies, df_links, on='movieId')
    
#     tittle = df_movies["movieId"][df_movies["title"]== film]
#     # tmdbId = df_links[df_links["tmdbId"] == movieId]
#     print(df_movies[["movieId","title"]])
#     print(df_movies[df_movies.title.eq(film)])
...
def get_sinopsis(title):
    # Leer el archivo CSV
    df_movies_sinopis = pd.read_csv("movies_sinopsis.csv")

    # Buscar la sinopsis en el archivo CSV
    for i, row in df_movies_sinopis.iterrows():
        if row["title"] == title:
            return row["sinopsis"]
    return "Sinopsis no encontrada"
